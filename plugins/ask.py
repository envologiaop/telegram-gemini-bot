...
from bot.ai import get_ai_response, forget_chat_history # We will create bot/ai.py next

logger = logging.getLogger(__name__)

@Client.on_message(filters.me & filters.command("ask", prefixes="."))
async def ask_ai_handler(client: Client, message: Message):
    """Handles the .ask command by getting a response from the AI module."""
    prompt = " ".join(message.command[1:])
    if not prompt:
        return await message.edit_text("`Usage: .ask <prompt>`")
    ...
    try:
        ...
        await message.edit_text("`An error occurred while getting Envo's response.`")
